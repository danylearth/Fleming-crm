...
describe('tenant completion',()=>{
  it('excludes guarantors unless explicitly required and counts legacy joint only without a linked record',()=>{
    expect(tenantCompletion({guarantor_required:false}).length).toBe(7);
    expect(tenantCompletion({guarantor_required:1}).length).toBe(9);
    expect(tenantCompletion({guarantor_required:'0'}).length).toBe(7);
    expect(tenantCompletion({is_joint_tenancy:true,first_name_2:'Partner'}).length).toBe(8);
    expect(tenantCompletion({is_joint_tenancy:true,first_name_2:'Partner'},2).length).toBe(7);
  });
