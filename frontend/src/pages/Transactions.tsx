...
export default function Transactions() {
  ...
  const [showOlderRent,setShowOlderRent]=useState(false);
  const rentCutoff=new Date();rentCutoff.setUTCDate(rentCutoff.getUTCDate()-30);
  const selectablePayments=payments.filter(p=>p.due_date&&(showOlderRent||p.due_date.slice(0,10)>=rentCutoff.toISOString().slice(0,10))&&p.due_date.slice(0,10)<=new Date().toISOString().slice(0,10)&&(!Number(p.bank_received)||Number(p.amount_paid)<Number(p.amount_due)));
  ...
  const beginAssign=async(transaction:BankFeedTransaction)=>{
    setShowOlderRent(false);setBankMessage('');setTransactionName(transaction.display_name||transaction.description||'');setShowImportName(false);
  ...
  return (
    <Layout title="Financials" breadcrumb={[{ label: 'Financials' }]}>
      {selectedBank&&<div role="dialog" aria-modal="true" aria-label="Assign bank transaction" className="fixed inset-0 z-[100] grid place-items-center bg-black/60 p-4"><div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-[var(--bg-card)] p-6 space-y-4">
        <h2 className="text-lg font-semibold">{selectedBank.match_status==='unmatched'?'Assign':'Review'} £{Math.abs(Number(selectedBank.amount)).toFixed(2)}</h2><div className="flex items-end gap-2"><Input className="flex-1" label="Transaction Name" value={transactionName} onChange={setTransactionName}/><Button variant="outline" size="sm" disabled={bankBusy||!transactionName.trim()} onClick={()=>void renameTransaction()}>Save Name</Button></div><button className="text-xs underline" onClick={()=>setShowImportName(v=>!v)}>{showImportName?'Hide Import Name':'View Import Name'}</button>{showImportName&&<p className="text-sm break-words">{selectedBank.description}</p>}
        ...
        {Number(selectedBank.amount)>0&&<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={showOlderRent} onChange={e=>setShowOlderRent(e.target.checked)}/>Include older rent charges</label>}
