...
import { Users, Plus, X, Phone, Mail, Home, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function Tenants() {
  ...
  const perPage = 10;
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', emergency_contact: '', notes: '' });

  useEffect(() => { loadData(); }, []);
  const loadData = () => { api.get('/api/tenants').then(setTenants).catch(console.error).finally(() => setLoading(false)); };
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{filtered.length} Tenants</span>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Emergency</th>
              </tr>
            </thead>
            <tbody>
              {paged.length === 0 ? (
                ...
                return (
                  <tr key={tenant.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                    <td className="w-10 px-3 py-3">
                      <input type="checkbox" checked={selectedIds.has(tenant.id)}
                    ...
                    <td className="px-4 py-3">
                      <Link to={`/tenants/${tenant.id}`} className="flex items-center gap-3 group">
                        ...
                        <p className="text-sm font-semibold text-gray-900 group-hover:text-gray-600">{tenant.name}</p>
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-500">
                      {tenant.current_property ? (
                        <span className="flex items-center gap-1.5"><Home className="w-3.5 h-3.5 text-gray-400" />{tenant.current_property}</span>
                      ) : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-400">{tenant.emergency_contact || <span className="text-gray-300">—</span>}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <div className="bg-white rounded-lg w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit"
                  className="px-4 py-2 bg-gray-900 text-white rounded-lg hover:bg-gray-800 text-sm font-medium">Add Tenant</button>
              </div>
            </form>
          </div>
        </div>
