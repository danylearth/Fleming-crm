...
it('uses the supplied team signatures and safely fills the selected user profile',()=>{
 const rows=emailSignatures([{id:1,name:'Sam <Fleming>',email:'sam@fleminglettings.co.uk',department:'Lettings',phone:'07700900123',office_extension:'42'}],1);
 expect(rows.map(r=>r.label)).toEqual(['Accounts Department','Office Support Team','Sam <Fleming>']);
 const signature=rows.find(r=>r.id==='user')!;expect(signature.html).toContain('Sam &lt;Fleming&gt;');expect(signature.html).toContain('Lettings');expect(signature.html).toContain('07700900123');expect(signature.html).toContain('ext. 42');expect(signature.html).toContain('13943597');expect(signature.html).not.toContain('{{');expect(signature.html).toContain('https://crm.fleminglettings.co.uk/email-assets/signature-logo-white.png');
});
