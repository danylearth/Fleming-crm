...
import {normalizePropertyAddress,rentReminderEmail,tenancyEndEmail} from './email';
export function registerTenantMessageTemplates(app:Express){
 app.get('/api/tenants/:id/message-templates',authMiddleware,requirePermission('staff'),async(req:AuthRequest,res)=>{
  const tenant=await queryOne('SELECT t.*,p.address AS property_address,p.postcode AS property_postcode FROM tenants t LEFT JOIN properties p ON p.id=t.property_id WHERE t.id=$1',[req.params.id]);
  if(!tenant)return res.status(404).json({error:'Tenant not found'});
  tenant.address=normalizePropertyAddress(tenant.property_address,tenant.property_postcode)||'Property not linked — contact the office';
