 ...
 app.post('/api/bank-feed/transactions/:id/reconcile',authMiddleware,requireFinance,requirePermission('staff'),async(req:AuthRequest,res)=>{
  ...
  try {
   ...
   if(action==='unassign') {
    ...
    for(const id of rentIds)await client.query('SELECT id FROM rent_payments WHERE id=$1 FOR UPDATE',[id]);
    ...
    if(action==='ignore')await client.query("UPDATE bank_feed_transactions SET match_status='ignored' WHERE id=$1",[bank.id]);
     ...
     for(const a of allocations) {
      ...
      if(a.kind==='rent') {
       ...
       if(a.property_id){const property=(await client.query('SELECT id FROM properties WHERE id=$1',[a.property_id])).rows[0];if(!property)throw new Error('Choose an existing property');propertyId=property.id;}
       ...
       if(!incoming&&propertyId)expenseId=(await client.query('INSERT INTO property_expenses(property_id,description,amount,category,expense_date,maintenance_id) VALUES($1,$2,$3,$4,$5,$6) RETURNING id',[propertyId,[bank.description,notes].filter(Boolean).join(' — ').slice(0,4000),amount,category,bank.booked_at,a.kind==='maintenance'?a.maintenance_id:null])).rows[0].id;
