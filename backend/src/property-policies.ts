...
export function registerPropertyPolicies(app:Express){
 ...
 app.post('/api/properties/:id/policies',authMiddleware,requirePermission('staff'),async(req:AuthRequest,res)=>{
  ...
  for(const key of ['policy_number','broker_name','broker_phone','broker_email'])if(d[key]&&String(d[key]).length>240)return res.status(400).json({error:'Policy and broker details must be under 240 characters.'});
  ...
  try{
   ...
   const policy=(await c.query('INSERT INTO property_policies(policy_type,policy_number,annual_cost,commencement_date,expiry_date,broker_name,broker_phone,broker_email,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *',[d.policy_type,d.policy_number||null,Number(d.annual_cost),d.commencement_date,d.expiry_date,d.broker_name||null,d.broker_phone||null,d.broker_email||null,req.user.id])).rows[0];
