...
export async function flemoEvidence(message:string,portfolio:string,context?:{entityType?:string;entityId?:number},readScans=false) {
  ...
  return {records,summary,documents:documentEvidence,paymentHistory,latestSms,portfolioSummary:{...scheduled[0],chargesDueThisCalendarMonth:monthly[0],currency:'GBP',jointTenantsCountOnce:true},catalog:catalog.map(record=>({id:record.id,name:record.name,entity:record.entity,status:record.status})),limitations:['Amounts come from recorded CRM charges and payments, not a bank balance.','No recorded payments means payment timeliness cannot be assessed.','Document uploads are evidence held, not verified identity or eligibility.']};
}
