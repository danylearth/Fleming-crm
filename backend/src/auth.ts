...
export interface AuthRequest extends Request {
  user?: {
    ...
    name: string;
    department?: string;
    phone?:string; office_extension?:string; contact_email?:string;
...
export async function authMiddleware(req: AuthRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const user = await queryOne(
      'SELECT is_active, password_setup_required, last_password_change, role, name, email, department, phone, office_extension, contact_email, finance_access, last_login, avatar_url, accent_color, appearance FROM users WHERE id = $1',
      [decoded.id]
    );
    ...
    req.user = { ...decoded, department: user.department, phone:user.phone, office_extension:user.office_extension, contact_email:user.contact_email, finance_access:user.finance_access, role: user.role, name: user.name, email: user.email, last_login: user.last_login, avatar_url: user.avatar_url, accent_color: user.accent_color, appearance: user.appearance };
    next();
  } catch (err) {
    return res.status(401).json({ error: 'Invalid token' });
  }
